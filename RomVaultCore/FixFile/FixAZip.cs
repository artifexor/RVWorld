...
namespace RomVaultCore.FixFile
{
    public static class FixAZip
    {
        ...
        public static ReturnCode FixZip(RvFile fixZip, List<RvFile> fileProcessQueue, ref int totalFixed, out string errorMessage)
        {
            ...
            // zip files should not internally need a case fix, this code was added due to another error.
            // that while scanning matched no-case matches in a zip file.
            bool needsCaseFixed = false;
            for (int iRom = 0; iRom < fixZip.ChildCount; iRom++)
                needsCaseFixed |= !string.IsNullOrEmpty(fixZip.Child(iRom).FileName);
            ...
            if (!fixZip.DirStatus.HasFixable() && !needsTrrntzipped && !needsCaseFixed)
            {
                return ReturnCode.Good;
            }
                ...
                for (int iRom = 0; iRom < fixZip.ChildCount; iRom++)
                {
                    ...
                    switch (fixZippedFile.RepStatus)
                    {
                        ...
                        case RepStatus.CorruptCanBeFixed:
                            {
                                returnCode = FixAZipFunctions.CanBeFixed(fixZip, fixZippedFile, ref tempFixZip, filesUsedForFix, ref totalFixed, out errorMessage);
                                if (returnCode != ReturnCode.Good)
                                {
                                    CloseZipFile(ref tempFixZip);
                                    CloseToSortGame(toSortGame, ref toSortZipOut);
